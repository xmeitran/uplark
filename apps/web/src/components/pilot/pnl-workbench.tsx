            ...
            <section id="pnl-detail" className="scroll-mt-4 space-y-5 rounded-2xl border border-border bg-card p-5 shadow-sm sm:p-7">
                ...
                <section className="overflow-hidden rounded-xl border border-amber-200 bg-amber-50/30">
                  <div className="border-b border-amber-200 px-4 py-4">
                    <div className="flex items-center justify-between gap-3">
                      <div><h3 className="text-sm font-bold">Delivery / DX · phân bổ theo nhân sự</h3><p className="mt-1 text-[11px] text-muted-foreground">Đối soát 3 lớp giờ cho từng thành viên thuộc nhóm Delivery / DX · P = baseline ngày, L = giờ ghi nhận, P&amp;L = giờ hợp lệ.</p></div>
                      <Status tone="warning">{deliveryDxPeople.length} nhân sự</Status>
                    </div>
                  </div>
                  <div className="overflow-x-auto">
                    <table className="w-full min-w-[1120px] text-left text-xs">
                      <thead className="bg-amber-50/60 text-[10px] uppercase tracking-wide text-muted-foreground"><tr><th className="px-4 py-3">Nhân sự / vai trò</th><th className="px-3 py-3 text-right">Plan Hour</th><th className="px-3 py-3 text-right">Logwork Hour</th><th className="px-3 py-3 text-right">P&amp;L Hour</th>{LOGWORK_DATES.slice(0, 10).map((date) => <th key={date} className="px-2 py-3 text-right"><div>{date.slice(0, 5)}</div><div className="mt-1 text-[9px] font-normal normal-case tracking-normal text-muted-foreground">P · L · P&amp;L</div></th>)}</tr></thead>
                      <tbody className="divide-y divide-amber-100">
                        {deliveryDxBreakdown.map((person) => <tr key={person.name} className="hover:bg-amber-50/60"><td className="px-4 py-3"><div className="font-semibold">{person.name}</div><div className="text-[10px] text-muted-foreground">{person.role}</div></td><td className="px-3 py-3 text-right font-mono">{person.plan.toFixed(1)}h</td><td className="px-3 py-3 text-right font-mono font-semibold text-emerald-700">{person.logwork.toFixed(1)}h</td><td className="px-3 py-3 text-right font-mono font-semibold text-violet-700">{person.pnl.toFixed(1)}h</td>{LOGWORK_DATES.slice(0, 10).map((date) => { const entry = dailyLogwork.find((item) => item.name === person.name && item.date === date); const logwork = entry?.hours ?? 0; const planDaily = Math.min(8, person.plan / LOGWORK_DATES.length); const pnlDaily = logwork * (project.pnl / Math.max(1, project.logwork)); return <td key={date} className="px-2 py-2 text-right font-mono text-[10px] leading-4"><div className="text-blue-700">{planDaily.toFixed(1)}</div><div className="font-semibold text-emerald-700">{logwork.toFixed(1)}</div><div className="font-semibold text-violet-700">{pnlDaily.toFixed(1)}</div></td>; })}</tr>)}
                        <tr className="bg-amber-50/70 font-bold"><td className="px-4 py-3">Tổng Delivery / DX</td><td className="px-3 py-3 text-right font-mono">{deliveryDxBreakdown.reduce((sum, person) => sum + person.plan, 0).toFixed(1)}h</td><td className="px-3 py-3 text-right font-mono text-emerald-700">{deliveryDxBreakdown.reduce((sum, person) => sum + person.logwork, 0).toFixed(1)}h</td><td className="px-3 py-3 text-right font-mono text-violet-700">{deliveryDxBreakdown.reduce((sum, person) => sum + person.pnl, 0).toFixed(1)}h</td>{LOGWORK_DATES.slice(0, 10).map((date) => { const logwork = dailyLogwork.filter((item) => item.date === date && deliveryDxPeople.some((person) => person.name === item.name)).reduce((sum, item) => sum + item.hours, 0); const planDaily = deliveryDxBreakdown.reduce((sum, person) => sum + Math.min(8, person.plan / LOGWORK_DATES.length), 0); const pnlDaily = logwork * (project.pnl / Math.max(1, project.logwork)); return <td key={date} className="px-2 py-2 text-right font-mono text-[10px] leading-4"><div className="text-blue-700">{planDaily.toFixed(1)}</div><div className="text-emerald-700">{logwork.toFixed(1)}</div><div className="text-violet-700">{pnlDaily.toFixed(1)}</div></td>; })}</tr>
                      </tbody>
                    </table>
                  </div>
